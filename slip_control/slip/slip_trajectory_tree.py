...
class TrajectoryTreeNode:
    ...
    def __init__(
        self,
        gait_cycle: Optional[SlipGaitCycleCtrl],
        child_nodes: Dict[(float, "TrajectoryTreeNode")],
        parent_node: Optional["TrajectoryTreeNode"],
        is_root=False,
    ):
    ...
    def get_optimal_trajectory(self) -> SlipTrajectory:
        ...
        opt_traj = SlipTrajectory(
            slip_model=(present_node.gait_cycle.slip_model),
            slip_gait_cycles=[copy.deepcopy(present_node.gait_cycle)],
        )
    ...
    @property
    def tree_size(self):
        return self.n_branches + sum(
            [node.tree_size for node in self.child_nodes.values()]
        )

    def __str__(self):
        return "n:%d_cost:%.3f_children:%d_future_branches:%d" % (
            self.cycle_number,
            self.gait_cycle_optimization_cost,
            self.n_branches,
            self.tree_size,
        )
    ...
    def plot(
        self,
        optimal_color=(0, 0, 0, 1),
        sub_optimal_color=(1, 0, 0, 0.15),
        unfeasible_color=(0, 0, 1, 0.15),
    ):
        ...
        outer_pbar = tqdm(
            total=n_cycles,
            desc="Trajectory tree plot | Gait Cycles",
            position=0,
            leave=True,
            file=(sys.stdout),
        )
        ...
        while len(present_node.child_nodes) > 0:
            cycle_start_time = (
                present_node.gait_cycle.start_time if not present_node.is_root else 0.0
            )
            optimal_node = present_node.optimal_child_node
            plt_axs = plot_slip_trajectory(
                (optimal_node.gait_cycle),
                color=optimal_color,
                plot_passive=True,
                plt_axs=plt_axs,
                plot_td_angles=True,
