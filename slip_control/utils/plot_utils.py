    ...
    if not traj.shape[0] == 6:
        ...
        if not traj.shape[(-1)] == t.size:
            raise AssertionError(
                "`t` %s and `traj` %s must have same lengths" % (t.shape, traj.shape)
            )
        else:
            if plt_axs is None:
            ...
            if area_color is not None:
                xz_ax.fill_between(
                    (traj[0, :]), (traj[3, :]), alpha=0.1, color=area_color
                )
            if foot_pos is not None:
                ...
                for i in range(n):
                    xz_ax.plot(
                        [foot_pos, yz_coarse_traj[(0, i)]],
                        [0, yz_coarse_traj[(3, i)]],
                        color=color,
                        alpha=0.15,
                        linestyle=linestyle,
                        marker="o",
        ...
        xz_ax.set_xlabel(
            "$\\mathbf{y[m]}$", fontweight="bold", fontsize=LABEL_FONT_SIZE
        )
        xz_ax.set_ylabel(
            "$\\mathbf{z[m]}$", fontweight="bold", fontsize=LABEL_FONT_SIZE
        )
        if put_legend:
...
def plot_polar_trajectory(
    polar_traj, t, target_state=None, axs=None, color="k", label="", target_color="b"
):
    if axs is None:
    ...
    if target_state is None:
        ...
        raise AssertionError
    y_titles = ["$r$", "$\\dot{r}$", "$\\ddot{r}$"]
    for i, ax in enumerate(r_axs.flatten()):
        ax.plot(
            t, (polar_traj[i + 2, :]), "k-", markersize="2", label=label, color=color
        )
        ax.plot(
            (t[0]),
            (polar_traj[(i + 2, 0)]),
        ...
        ax.plot(
            (t[(-1)]), (target_state[(i + 2)]), "D", color=target_color, markersize="10"
        )
...
def plot_control_trajectory(
    ...
    ax.fill_between(
        (theta * 180 / np.pi), resultant_force, passive_force, alpha=0.1, color=color
    )
...
def plot_slip_trajectory(
    slip_traj: Union[(SlipTrajectory, SlipGaitCycleCtrl)],
    color="k",
    plot_passive=True,
    plt_axs=None,
    title=None,
    ...
    for gait_cycle in cycles:
        touch_down_angle = (
            np.rad2deg(gait_cycle.touch_down_angle) if plot_td_angles else None
        )
        # Plot flight trajectory
        fig, plt_axs = plot_cartesian_trajectory(
            traj=(gait_cycle.flight_cartesian_traj),
            t=(gait_cycle.t_flight),
            color=color,
            plt_axs=plt_axs,
            linestyle="dotted",
        ...
        if (
            plot_passive
            and isinstance(gait_cycle, SlipGaitCycleCtrl)
            and gait_cycle.passive_gait_cycle is not None
        ):
            passive_cycle = gait_cycle.passive_gait_cycle
            fig, plt_axs = plot_cartesian_trajectory(
                traj=(passive_cycle.stance_cartesian_traj),
        ...
        fig, plt_axs = plot_cartesian_trajectory(
            traj=(gait_cycle.stance_cartesian_traj),
            t=(gait_cycle.t_stance),
            color=color,
            plt_axs=plt_axs,
            linestyle="-",
...
def plot_limit_cycles(
    slip_traj: SlipTrajectory, axs=None, cmap_name="copper", fig_size=(15, 10)
):
    LABEL_FONT_SIZE = 12
    cmap = plt.cm.get_cmap(cmap_name)

    def plot_limit_cycle_metric(
        x_flight,
        y_flight,
        x_stance,
        y_stance,
    ...
    for i, cycle in enumerate(slip_traj.gait_cycles):
        ...
        flight_phase = slip_traj.get_gait_phase(
            np.linspace(
                start=cycle.t_flight[0],
                stop=cycle.t_flight[-1],
                num=len(cycle.t_flight),
            )
        )
        stance_phase = slip_traj.get_gait_phase(
            np.linspace(
                start=cycle.t_stance[0],
                stop=cycle.t_stance[-2],
                num=len(cycle.t_stance),
            )
        )

        plot_limit_cycle_metric(
            x_flight=(flight_traj[X_DOT]),
            y_flight=(flight_traj[Z_DOT]),
        ...
        plot_limit_cycle_metric(
            x_flight=(flight_traj[X_DDOT]),
            y_flight=(flight_traj[Z_DDOT]),
            x_stance=(stance_traj[X_DDOT]),
            y_stance=(stance_traj[Z_DDOT]),
            x_name=r"$\ddot{x}$ [m/s$^2$]",
            y_name=r"$\ddot{z}$ [m/s$^2$]",
            flight_phase=flight_phase,
            stance_phase=stance_phase,
            ax=(axs[(1, 0)]),
            ax2=(axs[(1, 1)]),
        ...
        plot_limit_cycle_metric(
            x_flight=None,
            y_flight=None,
            x_stance=(np.rad2deg(stance_traj_polar[THETA])),
        ...
        plot_limit_cycle_metric(
            x_flight=None,
            y_flight=None,
            x_stance=(np.rad2deg(stance_traj_polar[THETA_DOT])),
