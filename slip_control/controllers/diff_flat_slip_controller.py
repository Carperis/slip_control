...
class SlipDiffFlatController:
    ...
    def __init__(
        ...
        traj_weights,
        target_to_state_generator: Optional[SlipTargetStateGenerator] = None,
        poly_degree=6,
        qp_grid_points=11,
        max_flight_theta_dot=np.inf,
        debug=False,
    ):
        if not 32 > poly_degree >= 6:
            raise AssertionError(
                "Minimum polynomial degree is 6 max is 32, 6 is enough!"
            )
        else:
            assert traj_weights.shape == (
                6,
            ), "Invalid costs shapes [%s], should (6,)" % str(traj_weights.shape)
            if target_to_state_generator is None:
                target_to_state_generator = SlipTargetStateGenerator(
                    slip_model, np.ones(6)
                )
    ...
    def generate_slip_trajectory_tree(
        self,
        initial_state,
        desired_gait_cycles=np.inf,
        desired_duration=np.inf,
        look_ahead_cycles=0,
        max_samples_per_cycle=20,
        angle_epsilon=np.deg2rad(1),
        verbose=True,
    ) -> TrajectoryTreeNode:
        """
        ...
        """
        assert xor(
            np.isinf(desired_gait_cycles), np.isinf(desired_duration)
        ), "Provide ONE stopping criteria: `desired_gait_cycles` or `desired_duration`"
        opt_start = time.time()
        ...
        root_node = TrajectoryTreeNode(
            gait_cycle=None, parent_node=None, child_nodes={}, is_root=True
        )
        root_node._init_to_state = initial_state
        present_node = root_node
        pbar = tqdm(
            total=(float(limit)),
            desc="Traj Optimization",
            dynamic_ncols=True,
            disable=(not verbose),
            position=0,
            leave=True,
        ...
        while counter <= limit:
            self.grow_trajectory_tree(
                present_node,
                look_ahead_cycles=look_ahead_cycles,
    ...
    def sample_new_angles(self, parent_node: TrajectoryTreeNode, min_angle_delta):
        ...
        if len(planned_trajectories) == 0:
            ...
            costs = [
                child_node.compute_opt_cost()
                for child_node in planned_trajectories.values()
            ]
            ...
            for candiate in angle_candidates:
                ...
                if candidate_idx != 0 and candidate_idx != len(sorted_angles) - 1:
                    # Spawn left
                    step = (
                        sorted_angles[candidate_idx]
                        - sorted_angles[(candidate_idx - 1)]
                    ) / 2
                    ...
                    if candidate_idx == 0:
                        if sorted_angles[candidate_idx] != MIN_TD_ANGLE:
                            new_angles.append(
                                max([sorted_angles[0] - np.deg2rad(5), MIN_TD_ANGLE])
                            )
                if candidate_idx != len(sorted_angles) - 1:
                    if sorted_angles[(-1)] != MAX_TD_ANGLE:
                        new_angles.append(
                            min([sorted_angles[(-1)] + np.deg2rad(5), MAX_TD_ANGLE])
                        )
                sorted_angles = sorted(sorted_angles + new_angles)

        if np.any(new_angles > MAX_TD_ANGLE) or np.any(new_angles < MIN_TD_ANGLE):
            raise Exception(
                "Sampled touch-down angles out of bounds (min:%.2f, max:%.2f): %s : "
                % (
                    np.rad2deg(MIN_TD_ANGLE),
                    np.rad2deg(MAX_TD_ANGLE),
                    np.rad2deg(new_angles),
                )
            )
        return new_angles

    def grow_trajectory_tree(
        self,
        parent_opt_node: TrajectoryTreeNode,
        look_ahead_cycles=0,
        gait_aux_variables: Optional[Dict] = None,
        min_angle_delta=np.deg2rad(1),
        max_samples=15,
    ) -> None:
        ...
        t_cycle_start = (
            parent_opt_node.gait_cycle.end_time if not parent_opt_node.is_root else 0.0
        )
        ...
        for theta_td, existing_node in parent_opt_node.child_nodes.items():
            if (
                not np.isinf(existing_node.gait_cycle_optimization_cost)
                and look_ahead_cycles > 0
            ):
                self.grow_trajectory_tree(
                    parent_opt_node=existing_node,
        ...
        while n_samples < max_samples:
            ...
            for theta_td in new_td_angles:
                x_coeff, z_coeff = self.get_warm_start_coefficients(
                    parent_opt_node, float(theta_td)
                )
                ...
                gait_cycle = self.gait_cycle_optimization(
                    to_init_state=parent_opt_node.final_to_state,
                ...
                child_node = TrajectoryTreeNode(
                    gait_cycle=gait_cycle, child_nodes={}, parent_node=parent_opt_node
                )
    ...
    def gait_cycle_optimization(
        self,
        to_init_state,
        theta_td,
        prev_to_angle,
        gait_aux_variables: Dict,
        prev_x_coeff=None,
        prev_z_coeff=None,
    ) -> Union[(SlipGaitCycle, SlipGaitCycleCtrl)]:
        assert (
            to_init_state.shape[0] == 6
        ), "Invalid Initial TO cartesian State Shape [%s] " % str(to_init_state.shape)
        assert (
            MAX_TD_ANGLE >= theta_td >= MIN_TD_ANGLE
        ), "Invalid Touch Down (TD) angle %.3f[Deg]" % np.rad2deg(theta_td)
        td_state_cartesian, tof, x_foot_TD_pos = self.slip_model.predict_td_state(
            to_init_state, theta_td
        )
        TD_state_polar, u_TD = self.slip_model.cartesian_to_polar(
            td_state_cartesian, foot_contact_pos=x_foot_TD_pos
        )
        # Get flight trajectory
        t_flight = np.linspace(start=0.0, stop=tof, num=10)
        flight_cart_traj = self.slip_model.get_flight_trajectory(
            t=t_flight, take_off_state=to_init_state
        )

        flight_theta_dot = (
            np.abs(theta_td - prev_to_angle) / tof if tof != 0.0 else np.inf
        )
        if (
            flight_theta_dot > self.max_flight_theta_dot
            or TD_state_polar[R] < self.slip_model.r0 * 0.98
        ):
            return SlipGaitCycle(
                slip_model=self.slip_model,
                t_flight=t_flight,
                flight_cartesian_traj=flight_cart_traj,
                t_stance=(np.array([tof])),
        ...
        t_stance, stance_passive_polar_traj = self.slip_model.get_stance_trajectory(
            TD_state_polar
        )
        ...
        stance_passive_cart_traj = self.slip_model.polar_to_cartesian(
            trajectory=stance_passive_polar_traj,
            control_signal=u_passive,
            foot_contact_pos=0.0,
        )

        if (
            stance_passive_cart_traj[(Z_DOT, -1)] < 0.0
            or stance_passive_cart_traj[(X_DOT, -1)] < 0.0
            or stance_time < expected_stance_duration * 0.5
        ):
            invalid_cycle = SlipGaitCycle(
                slip_model=self.slip_model,
                t_flight=t_flight,
                flight_cartesian_traj=flight_cart_traj,
                t_stance=t_stance,
        ...
        to_des_state = self.target_to_state_generator.get_target_cartesian_state(
            td_state=td_state_cartesian,
            prev_to_state=to_init_state,
            stance_time=stance_time,
            **gait_aux_variables
        )
        # Generate target take-off state error weights
        to_des_state_weights = self.target_to_state_generator.get_target_state_weights(
            **gait_aux_variables
        )
        ...
        if self.qp_grid_points < stance_passive_cart_traj.shape[1]:
            coarse_idx = np.round(
                np.linspace(
                    0, stance_passive_cart_traj.shape[1] - 1, self.qp_grid_points
                )
            ).astype(int)
        ...
        x_coeff, z_coeff, opt_cost = self.solve_QP(
            to_des_state=to_des_state,
            to_des_state_weights=to_des_state_weights,
        ...
        stance_controlled_cart_traj = self.get_optimal_trajectory(
            x_coeff, z_coeff, times=(t_stance - t_stance[0])
        )
        stance_controlled_cart_traj = np.array(stance_controlled_cart_traj)
        stance_controlled_polar_traj, u_optimal = self.slip_model.cartesian_to_polar(
            stance_controlled_cart_traj, foot_contact_pos=0.0
        )
        if self.debug:
            ...
            print(
                "QP reported new_start_time: %.4f[s]" % self.qp.solver_stats.solve_time
            )
            ...
            print(
                "Stance Passive Trajectory (cartesian) Shape:",
                stance_passive_cart_traj.shape,
            )
            print(
                "Stance Controlled Trajectory (cartesian) Shape:",
                stance_controlled_cart_traj.shape,
            )
            print(
                "Stance Coarse Controlled Trajectory (cartesian) Shape:",
                coarse_stance_passive_cart_traj.shape,
            )

        return SlipGaitCycleCtrl(
            slip_model=(self.slip_model),
            ...
            ctrl_kwargs={"x_coeff": x_coeff, "z_coeff": z_coeff},
        )

    def solve_QP(
        self,
        to_des_state,
        to_des_state_weights,
        td_init_state,
        passive_trajectory,
        traj_times,
        prev_x_coeff=None,
        ...
        assert (
            passive_trajectory.shape[0] == to_des_state.shape[0] == 6
        ), "[x, x', x'', z, z', z'']"
        num_ref_traj_points = traj_times.shape[0]
        assert (
            self.qp_grid_points >= num_ref_traj_points > 0
        ), "Invalid passive trajectory length"
        ...
        x_coeff_init[:3, 0] = [
            td_init_state[X],
            td_init_state[X_DOT],
            td_init_state[X_DDOT] / 2,
        ]
        z_coeff_init = np.zeros((self.degree + 1, 1))
        z_coeff_init[:3, 0] = [
            td_init_state[Z],
            td_init_state[Z_DOT],
            td_init_state[Z_DDOT] / 2,
        ]
        ...
        for ref_grid_point in range(num_ref_traj_points):
            ...
            Q_max_sqrt = (
                left_eigen_vectors * np.sqrt(eigen_values) @ left_eigen_vectors.T
            )
        ...
        if num_ref_traj_points < self.qp_grid_points:
            ...
            for missing_grid_point in range(
                num_ref_traj_points, self.qp_grid_points - 1
            ):
                phi_diag_params[missing_grid_point].value = np.zeros(
                    phi_diag_params[missing_grid_point].shape
                )
                B_params[missing_grid_point].value = np.zeros(
                    B_params[missing_grid_point].shape
                )
                C_params[missing_grid_point].value = np.zeros(
                    C_params[missing_grid_point].shape
                )
                Q_max_sqrt_params[missing_grid_point].value = np.zeros(
                    Q_max_sqrt_params[missing_grid_point].shape
                )
    ...
    def construct_QP_problem(self):
        ...
        lambd = cp.vstack(
            (x_coeff_var, z_coeff_var)
        )  # Variable of polynomial coefficients of x and z dims
        ...
        for grid_point in range(self.qp_grid_points):
            ...
            Q_max_sqrt = cp.Parameter(
                shape=(2 * (self.degree + 1), 2 * (self.degree + 1)),
                PSD=True,
            ...
            phi_diag_param = cp.Parameter(
                shape=(6, 2 * (self.degree + 1)), name=("phi_diag_%d" % grid_point)
            )
            phi_diag_param.value = np.zeros(phi_diag_param.shape)
            # Allocate resultant quadratic `b` matrix value
            # b = y_ref.T @ Q @ phi_diag
            b = cp.Parameter(
                shape=(1, 2 * (self.degree + 1)), name=("x_ref.Q.phi_%d" % grid_point)
            )
        ...
        assert prob.is_dcp(
            dpp=True
        ), "Problem is not DPP, recursive solving will not be efficient"
        assert prob.is_dcp(
            dpp=False
        ), "Problem is not DCP, it will either be infeasible or solved with a wrong solver"

        self.qp = prob
        self.qp_vars = {"x_coeff": x_coeff_var, "z_coeff": z_coeff_var}
        self.qp_params = {
            "phi_diag": phi_diag_params,
            "Q_max_sqrt": Q_max_sqrt_params,
    ...
    @staticmethod
    def get_warm_start_coefficients(
        parent_trajectory_node, theta_td
    ) -> [np.ndarray, np.ndarray]:
        x_coeff, z_coeff = (None, None)
        planned_angles = np.array(
            [
                k
                for k, node in parent_trajectory_node.child_nodes.items()
                if not np.isinf(node.gait_cycle_optimization_cost)
            ]
        )
        if len(planned_angles) != 0:
            closest_angle_idx = np.argmin(np.abs(planned_angles - theta_td))
            gait_cycle = parent_trajectory_node.child_nodes[
                planned_angles[closest_angle_idx]
            ].gait_cycle
        else:
        ...
        if isinstance(gait_cycle, SlipGaitCycleCtrl):
            if "x_coeff" in gait_cycle.ctrl_kwargs:
                x_coeff, z_coeff = (
                    gait_cycle.ctrl_kwargs["x_coeff"],
                    gait_cycle.ctrl_kwargs["z_coeff"],
                )
